...
// ── Group referral posts by company + role ────────────────
  ...
  for (const post of posts) {
    ...
    if (!map[key]) {
      map[key] = {
        ...
        bonusMin: post.bonusMin,
        bonusMax: post.bonusMax,
        posts: [],
        totalSlots: 0,
      };
    }
    ...
    // Use widest CTC range across all posts
    map[key].ctcMin = Math.min(map[key].ctcMin || post.ctcMin, post.ctcMin || 0);
    map[key].ctcMax = Math.max(map[key].ctcMax || post.ctcMax, post.ctcMax || 0);
    // Merge skills
    for (const s of (post.skills || [])) {
      if (!map[key].skills.includes(s)) map[key].skills.push(s);
    }
    // Highest bonus
    map[key].bonusMin = Math.max(map[key].bonusMin || 0, post.bonusMin || 0);
    map[key].bonusMax = Math.max(map[key].bonusMax || 0, post.bonusMax || 0);
  }
  return Object.values(map);
}

// ── Pick best employee to route to ───────────────────────
// Best = most slots available first, then earliest post
function pickBestEmployee(posts) {
  return [...posts].sort((a, b) => (b.slots || 1) - (a.slots || 1))[0];
}
...
export default function Browse() {
  ...
  const [appliedKeys, setAppliedKeys] = useState(new Set()); // tracks company+role already applied
    ...
    const fetch = async () => {
      try {
        ...
        const posts = snap.docs
          ...
          .filter(p => !p.lastDate || new Date(p.lastDate) >= today); // filter expired
        setGroups(groupPosts(posts));

        // If user logged in, fetch their existing applications
        if (auth.currentUser) {
  ...
  const handleApply = async (group) => {
    ...
    try {
      // Pick best employee
      const best = pickBestEmployee(group.posts);
      await addDoc(collection(db, "applications"), {
        ...
        routedBy: "smart-match", // mark as auto-routed
      });
  ...
  const allSkills = [...new Set(groups.flatMap(g => g.skills))].sort();
  const skillOptions = ["All Skills", ...allSkills];
  ...
  const isApplied = (group) => appliedKeys.has(`${group.company}__${group.role.toLowerCase().trim()}`);
  ...
  return (
    <div style={{ background: BG, minHeight: "100vh", color: WT, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ background: S1, borderBottom: `1px solid ${BR}`, padding: "12px 20px", position: "sticky", top: 60, zIndex: 90 }}>
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
          ...
          <select value={skill} onChange={e => setSkill(e.target.value)}
            style={{ background: S2, border: `1px solid ${BR}`, borderRadius: 8, padding: "7px 12px", color: WT, fontSize: 12, outline: "none", fontFamily: "inherit", appearance: "none", cursor: "pointer", flexShrink: 0 }}>
            {skillOptions.map(s => <option key={s} value={s} style={{ background: S2 }}>{s}</option>)}
          </select>
        </div>
      ...
      {/* HOW SMART ROUTING WORKS — info banner */}
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 14 }}>
            {filtered.map(group => {
              ...
              const employeeCount = group.posts.length;
              return (
                <div key={group.key}
                  onClick={() => { setSelected(group); setModalStep(1); }}
                  ...
                  {/* EMPLOYEE COUNT BADGE — key differentiator */}
                  <div style={{ background: `${G}10`, border: `1px solid ${G}22`, borderRadius: 8, padding: "8px 12px", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div>
                      <span style={{ color: G, fontWeight: 700, fontSize: 13 }}>{employeeCount} employee{employeeCount > 1 ? "s" : ""}</span>
                  ...
                  {/* BOTTOM ROW */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 10, borderTop: `1px solid ${BR}` }}>
                    <div style={{ display: "flex", gap: 14 }}>
                      <div><div style={{ color: MT, fontSize: 9, marginBottom: 2 }}>SALARY</div><div style={{ color: WT, fontWeight: 700, fontSize: 13 }}>₹{group.ctcMin}–{group.ctcMax}L</div></div>
                      <div><div style={{ color: MT, fontSize: 9, marginBottom: 2 }}>EXP</div><div style={{ color: WT, fontWeight: 700, fontSize: 13 }}>{group.expMin}–{group.expMax} yrs</div></div>
                    </div>
                    <div style={{ background: `${G}15`, border: `1px solid ${G}33`, borderRadius: 8, padding: "5px 10px", textAlign: "center" }}>
                      ...
                      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 14, fontWeight: 700, color: G }}>₹{Math.round((group.bonusMin || 30000) / 1000)}k+</div>
                    </div>
                  </div>
          ...
          <div style={{ background: S1, border: `1px solid ${BR}`, borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 480, padding: "28px 24px 40px", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            {modalStep === 1 && <>
              ...
              <div style={{ background: `${G}10`, border: `1px solid ${G}33`, borderRadius: 10, padding: "12px 14px", marginBottom: 16 }}>
                ...
                <div style={{ color: MT, fontSize: 12, lineHeight: 1.6 }}>
                  <strong style={{ color: WT }}>{selected.posts.length} employee{selected.posts.length > 1 ? "s" : ""}</strong> from {selected.company} can refer you. RaYa will automatically send your profile to the employee with the most available slots for the fastest response.
                </div>
              </div>

              {/* Job details */}
              {[
                ["CTC Range", `₹${selected.ctcMin}–${selected.ctcMax} LPA`],
                ["Experience", `${selected.expMin}–${selected.expMax} years`],
                ["Total Slots", `${selected.totalSlots} available`],
                ["Referral Bonus", `₹${Math.round((selected.bonusMin || 30000) / 1000)}k–₹${Math.round((selected.bonusMax || 75000) / 1000)}k`],
              ].map(([k, v]) => (
              ...
              <div style={{ background: `${G}0A`, border: `1px solid ${G}22`, borderRadius: 10, padding: "12px 14px", margin: "16px 0", fontSize: 12, color: MT, lineHeight: 1.7 }}>
                Referred candidates are <strong style={{ color: WT }}>5x more likely</strong> to get hired than direct applicants.
              </div>

              {isApplied(selected) ? (
                <div style={{ background: "#0A1F0A", border: "1px solid #1F5C1F", borderRadius: 10, padding: "14px", textAlign: "center", color: "#4ADE80", fontWeight: 700 }}>
                  ✓ Already Applied to this role at {selected.company}
                </div>
              ) : (
                <button onClick={() => auth.currentUser ? setModalStep(2) : nav("/login")}
                  style={{ width: "100%", background: `linear-gradient(135deg,${G},${GL})`, border: "none", color: BG, padding: "14px", borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            ...
            {modalStep === 2 && <>
              ...
              <div style={{ background: `${G}0A`, border: `1px solid ${G}22`, borderRadius: 12, padding: 20, marginBottom: 20, textAlign: "center" }}>
                ...
                <div style={{ color: MT, fontSize: 11, marginTop: 4 }}>Apply to unlimited roles • One application per company role</div>
              </div>

              <div style={{ background: S2, border: `1px solid ${BR}`, borderRadius: 10, padding: "12px 14px", marginBottom: 20, fontSize: 12, color: MT, lineHeight: 1.7 }}>
                ✦ You will <strong style={{ color: WT }}>not be charged twice</strong> for the same company + role. If you already applied to another Cognizant role earlier, this is a separate application.
              </div>

              <button onClick={() => handleApply(selected)} disabled={applying}
                style={{ width: "100%", background: `linear-gradient(135deg,${G},${GL})`, border: "none", color: BG, padding: "14px", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", marginBottom: 10, opacity: applying ? 0.7 : 1 }}>
                {applying ? "Applying..." : "Pay ₹799 & Apply"}
              </button>
              <button onClick={() => setModalStep(1)}
                style={{ width: "100%", background: "none", border: "none", color: MT, cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>← Go Back</button>
            </>}
            ...
            {modalStep === 3 && <>
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <div style={{ background: `${G}10`, border: `1px solid ${G}33`, borderRadius: 10, padding: "14px 16px", margin: "16px 0", textAlign: "left" }}>
                  ...
                  <div style={{ color: MT, fontSize: 12, lineHeight: 1.7 }}>
                    Your profile was routed to the <strong style={{ color: WT }}>best available employee</strong> at {selected.company} with the most referral slots. They will review your profile and submit internally.
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  <button onClick={() => { setSelected(null); nav("/dashboard"); }}
                    style={{ background: `linear-gradient(135deg,${G},${GL})`, border: "none", color: BG, padding: "12px", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                    Track Application →
                  </button>
